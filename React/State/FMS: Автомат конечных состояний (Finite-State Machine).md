...
__Идея конечного автомата очень проста:__ это такая сущность, которая в любой момент времени может находиться только в одном из `"глобальных"` состояний, число которых ограничено.
...
2. Механизм перехода состояний;
3. Удобный функционал отображения состояния машины;
4. Только одно активное состояний.
