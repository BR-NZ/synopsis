...
Данный компонент используется для управления рендерингом между переходами состояния в зависимости от выбранного режима `mode` и дочернего ключа `key`. Дочерним компонентом могут быть компонент `<Transition>` или `<CSSTransition>` – `<SwitchTransition>` выполняет согласованный переход между ними.
...
#### Механика работы
Меняя каким-либо образом дочернему компоненту `<Transition>` или `<CSSTransition>` пропс `key` мы заставляем `React` думать что это уже другой (новый) компонент.
А уже `<SwitchTransition>` делают работу по красивой замене компонента "старого" на "новый" заданным в `mode` образом.
