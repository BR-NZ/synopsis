...
* __`appear`__ – по умолчанию первое вхождение элемента при загрузке и при первом монтировании не анимируется, независимо от значения `in`. Для изменения этого поведения, следует установить `appear={true}` и `in={true}`;
