...
* `values` — объект по структуре схожий с `initialValues`, его свойства это `name`-атрибуты элементов формы. А в значениях он __содержит текущее значение каждого элемента__;
* `errors` — объект по структуре схожий с `initialValues`, его свойства это `name`-атрибуты элементов формы. А в значениях он __содержит ошибки для каждого элемента__;
* `touched` — объект по структуре схожий с `initialValues`, его свойства это `name`-атрибуты элементов формы. А в значениях он __содержит boolean, говорящий о срабатывании onBlur события__ на элементе. То есть в touched объекте поля помечаются как использованные, это нужно чтобы не валидировать поле до его использования пользователем.
