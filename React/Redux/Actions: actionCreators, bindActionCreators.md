...
* Это позволяет оптимизировать процесс создания действий, ведь теперь в `dispatcher` передается не объект, а вызов функции-создателя;
* Ошибка в вызове функции-создателе будет сразу видно (при обычной передаче объекта-действия, опечатка себя никак не покажет);
