...
### Прототипное наследование (прототип у прототипа)
...
### Нельзя наследовать от нескольких
Конечно, объекты могут иметь связь `[[Prototype]]` __только с одним другим объектом__. Нет возможности наследоваться от двух и более объектов.

### Из прототипа только чтение
Прототип используется только для чтения свойств. Операции записи/удаления не используют прототипы - они работают напрямую с объектом.

### Свойства прототипа перебираемы
По умолчанию, свойства в прототипе являются перебираемыми - их флаг `enumerable: true`. Кроме прототипов у созданных классом объектов.
